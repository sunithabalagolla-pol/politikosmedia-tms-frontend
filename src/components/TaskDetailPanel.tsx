...
import { X, Calendar, Flag, Upload, Check, ChevronDown, Download, Trash2, Loader2, Eye, Send, MessageSquare, AlertCircle, Pencil, Plus } from 'lucide-react'
...
import { useToggleSubtask, useDeleteSubtask, useAddSubtask, useUpdateSubtask } from '../hooks/api/useSubtasks'
...
export default function TaskDetailPanel({ isOpen, onClose, taskId, hideStatusDropdown, hidePriorityDropdown }: TaskDetailPanelProps) {
  ...
  const task = raw ? {
    ...
    deletedSubtasks: (raw.deleted_subtasks || []) as Array<{ id: number; title: string; created_by_name: string; deleted_at: string; deletion_note?: string }>,
  ...
  const canViewSubtasks = usePermission('subtask:view')
  const canCreateSubtask = usePermission('subtask:create')
  const canUpdateSubtask = usePermission('subtask:update')
  const canDeleteSubtask = usePermission('subtask:delete')
  ...
  const handleSubtaskDeleteClick = (subtaskId: number) => {
  ...
  const handleDeletionSubmit = async () => {
    ...
    try {
      await deleteSubtask.mutateAsync({ subtaskId: pendingDeleteSubtaskId, deletionNote: deletionNote.trim() || undefined })
      setShowDeletionModal(false)
      setDeletionNote('')
      setPendingDeleteSubtaskId(null)
    } catch {
      // hook invalidates queries; error surfaced by React Query
    }
  }
  ...
  const handleAddSubtask = async () => {
    ...
    try {
      await addSubtaskMutation.mutateAsync({ taskId: task.id, title: newSubtaskTitle.trim() })
      setNewSubtaskTitle('')
  ...
  const handleUpdateSubtask = async (subtaskId: number) => {
    ...
    try {
      await updateSubtaskMutation.mutateAsync({ id: subtaskId, title: editingSubtaskTitle.trim() })
  ...
  return (
    <>
          ...
          {task && (
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
              ...
              {canViewSubtasks && (
                <div>
                  <div className="flex items-center justify-between mb-0.5">
                    <h4 className="text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Subtasks</h4>
                    {canCreateSubtask && !showAddSubtask && (
                      <button
                        onClick={() => setShowAddSubtask(true)}
                        className="flex items-center gap-0.5 text-[9px] text-[#b23a48] hover:text-[#8f2e3a] font-medium transition-colors"
                      >
                  ...
                  {task.subtasks.length === 0 && !showAddSubtask ? (
                    <p className="text-[10px] text-gray-400 bg-gray-50 dark:bg-gray-700/50 rounded p-1.5 border border-gray-200 dark:border-gray-700 text-center">No subtasks</p>
                  ) : (
                    <div className="space-y-0.5 bg-gray-50 dark:bg-gray-700/50 rounded p-1.5 border border-gray-200 dark:border-gray-700">
                          ...
                          {editingSubtaskId === st.id ? (
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                className="flex-1 px-1 py-0.5 text-[10px] border border-[#b23a48] rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none"
                                autoFocus
                              />
                              <button onClick={() => handleUpdateSubtask(st.id)} disabled={updateSubtaskMutation.isPending || !editingSubtaskTitle.trim()} className="text-[9px] text-[#b23a48] font-medium px-1 disabled:opacity-50">
                                {updateSubtaskMutation.isPending ? '...' : 'Save'}
                              </button>
                              <button onClick={() => { setEditingSubtaskId(null); setEditingSubtaskTitle('') }} className="text-[9px] text-gray-400 font-medium px-1">
                                Cancel
                              </button>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 group/row">
                              <div onClick={() => handleSubtaskToggle(st)}
                                className="flex-1 flex items-center gap-1.5 text-[10px] cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 rounded px-1 py-0.5 transition-colors">
                              ...
                              {canUpdateSubtask && !st.completed && (
                                <button
                                  onClick={() => { setEditingSubtaskId(st.id); setEditingSubtaskTitle(st.title) }}
                                  className="shrink-0 text-gray-300 dark:text-gray-600 hover:text-blue-500 dark:hover:text-blue-400 p-0.5 rounded transition-colors opacity-0 group-hover/row:opacity-100"
                                  title="Edit subtask"
                                >
                              ...
                              {canDeleteSubtask && (
                                <button
                                  onClick={() => handleSubtaskDeleteClick(st.id)}
                                  className="shrink-0 text-gray-300 dark:text-gray-600 hover:text-red-500 dark:hover:text-red-400 p-0.5 rounded transition-colors opacity-0 group-hover/row:opacity-100"
                      ...
                      {canCreateSubtask && showAddSubtask && (
                        <div className="flex items-center gap-1 pt-0.5 border-t border-gray-200 dark:border-gray-600 mt-0.5">
                          <input
                            ...
                            className="flex-1 px-1 py-0.5 text-[10px] border border-[#b23a48] rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none"
                            autoFocus
                          />
                          <button onClick={handleAddSubtask} disabled={addSubtaskMutation.isPending || !newSubtaskTitle.trim()} className="text-[9px] text-[#b23a48] font-medium px-1 disabled:opacity-50">
                            {addSubtaskMutation.isPending ? '...' : 'Add'}
                          </button>
                          <button onClick={() => { setShowAddSubtask(false); setNewSubtaskTitle('') }} className="text-[9px] text-gray-400 font-medium px-1">
                            Cancel
                          </button>
                        </div>
                      )}
              ...
              {task.deletedSubtasks.length > 0 && (
                <div>
                  <h4 className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-0.5">Deleted Subtasks</h4>
                  <div className="space-y-0.5 bg-red-50/50 dark:bg-red-900/10 rounded p-1.5 border border-red-100 dark:border-red-900/30">
                    {task.deletedSubtasks.map((st) => (
                      <div key={st.id} className="px-1 py-0.5">
                        <div className="flex items-start gap-1.5">
                          <Trash2 className="w-2.5 h-2.5 text-red-400 shrink-0 mt-0.5" />
                          <div className="flex-1 min-w-0">
                            <span className="text-[10px] text-gray-400 dark:text-gray-500 line-through">{st.title}</span>
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                              <span className="text-[9px] text-gray-400">Deleted by {st.created_by_name}</span>
                              <span className="text-[9px] text-gray-400">·</span>
                              <span className="text-[9px] text-gray-400">{formatDate(st.deleted_at)}</span>
                            </div>
                            {st.deletion_note && (
                              <p className="text-[9px] text-red-500 dark:text-red-400 mt-0.5 italic">"{st.deletion_note}"</p>
                            )}
                          </div>
                        </div>
                      </div>
      ...
      {/* Subtask Deletion Note Modal */}
      {showDeletionModal && (
        <>
          <div className="fixed inset-0 bg-gray-900/50 dark:bg-black/70 z-[60]" onClick={() => { setShowDeletionModal(false); setDeletionNote(''); setPendingDeleteSubtaskId(null) }} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-2xl z-[70] p-6">
            <h3 className="text-[11px] font-bold text-gray-900 dark:text-white mb-1">Delete Subtask</h3>
            <p className="text-[11px] text-gray-500 dark:text-gray-400 mb-4">Optionally provide a reason for deleting this subtask.</p>
            <textarea
              value={deletionNote}
              onChange={(e) => setDeletionNote(e.target.value)}
              placeholder="e.g., Duplicate subtask, no longer required..."
              rows={3}
              className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg text-[11px] text-gray-900 dark:text-white bg-white dark:bg-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-400/20 focus:border-red-400 resize-none"
            />
            <div className="flex items-center gap-3 mt-4">
              <button
                onClick={() => { setShowDeletionModal(false); setDeletionNote(''); setPendingDeleteSubtaskId(null) }}
                className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-[11px] font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-[11px] font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
